# Wildcard Design System — v1.5 (dev-handoff variation)
...
**v1.5 = a duplicate of v1 evolving toward developer handoff — v1 stays frozen as the reference.** Per the dev team's input, everything here moves to: **vanilla only** (plain JS + HTML + CSS, no React, no build step), **per-animation standalone extractions** under [`handoff/`](handoff/) instead of the 6,000-line animated-homepage monolith, and **editor tooling stripped** (the "Tweaks" slider panel is design-iteration UI, not the website).

| Handoff deliverable | Status |
|---|---|
| [`handoff/icons-physics/`](handoff/icons-physics/) — hero icons ambient physics | ✅ ready |
| Full homepage animation set (fade-slide, 3D tilt, marquees, parallax, CTA pulse, smooth scroll) | ⏳ next |

The three variations in this repo: `v1` (HTML/animated reference, frozen) · `v1.5` (this folder — dev-handoff vanilla) · `v3` (React + Tailwind dashboard). Everything below this line is the duplicated v1 documentation, aligned step by step.
